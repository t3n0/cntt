...
## Note on the normalisation factors

The $\tilde N_{ij}^{\mathbf{k}} = \lambda_i^{\mathbf{k}*} \lambda_j^{\mathbf{k}} N$ normalisation factors can be absorbed into the definition of the $C_i^{n\mathbf{k}}$ coefficients. In this way we redefine the expansion coefficients so that they contain the normalisation of the Bloch functions

$$
\begin{equation}
\tilde C_i^{n\mathbf{k}} = \lambda_i^{\mathbf{k}} C_i^{n\mathbf{k}}.
\end{equation}
$$

Following this redefinition, the other quantities become

$$
\begin{equation}
\boxed{
\begin{split}
\psi_{n\mathbf{k}} (\mathbf{r}) &= \sum_{j} \tilde C_{j \mathbf{k}}^n \tilde \chi_{j \mathbf{k}}(\mathbf{r}), \\
\tilde \chi_{j \mathbf{k}}(\mathbf{r}) &= \frac{1}{\sqrt{N}} \sum_{\mathbf{R}} e^{i\mathbf{k} \cdot (\mathbf{R} + \mathbf{t}_{j})} \phi_{j}(\mathbf{r}-\mathbf{R}), \\
\tilde H^{\mathbf{k}}_{ij} &= e^{i \mathbf{k} \cdot (\mathbf{t}_{j} - \mathbf{t}_{i})}
\sum_{\mathbf{R}} e^{i\mathbf{k} \cdot \mathbf{R}} H_{ij} (\mathbf{R}), \\
\tilde S^{\mathbf{k}}_{ij} &= e^{i \mathbf{k} \cdot (\mathbf{t}_{j} - \mathbf{t}_{i})}
\sum_{\mathbf{R}} e^{i\mathbf{k} \cdot \mathbf{R}} S_{ij} (\mathbf{R}),
\end{split}}
\end{equation}
$$

The eigenvalue problem that we obtain is equivalent and the energies $E_{n\mathbf{k}}$ are exactly the same (but in terms of the tilde quantities):

$$
\begin{equation}
\boxed{
\begin{split}
\tilde H^{\mathbf{k}} \tilde C^{n\mathbf{k}} = E_{n\mathbf{k}} \tilde S^{\mathbf{k}} \tilde C^{n\mathbf{k}} \\
\det \left( \tilde H^{\mathbf{k}} - E_{n\mathbf{k}} \tilde S^{\mathbf{k}} \right) = 0
\end{split}}
\end{equation}
$$
...
1. By redefining the expansion coefficients $C^{n\mathbf{k}}$, we get rid of the burden of calculating the normalisation factors.
2. However, this is not equivalent to the **minimal TB approximation**: the overlap matris $\tilde S^{\mathbf{k}}$ is not a Kronecker delta.
3. This is clear by noticing that the $\tilde \chi_{j\mathbf{k}}$ Bloch functions are not normalised, but the total single-electron wavefunction $\psi_{n\mathbf{k}}$ is!
4. Finally, the energies that we obtain by solving the tilde eigenvalue problem are the same as in the non-tilde problem. However, I'm not quite sure about the meaning of the tilde $\tilde C^{n\mathbf{k}}$ lol
